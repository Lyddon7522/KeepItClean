...
public class Repository<T> : IRepository<T> where T : class
{
    ...
    public async Task<List<T>> GetAllAsync(IEnumerable<ScanCondition> scanConditions, CancellationToken cancellationToken) => await _dbContext.ScanAsync<T>(scanConditions).GetRemainingAsync(cancellationToken);
